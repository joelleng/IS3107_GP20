...
def get_mileage(json_data):
    ...
    return mileage if mileage != 0 else np.nan # N.A. mileage is reflected as 0 in scripts, we cannot take 0 as the true mileage is not available
...
def get_manufactured_year_soup(soup):
    ...
    return np.nan  # Return np.nan if "Manufactured" is not found
...
def get_road_tax_soup(soup):
    ...
    return np.nan  # Return np.nan if "Road Tax" is not found
...
def get_transmission_soup(soup):
    ...
    return np.nan  # Return np.nan if "Transmission" is not found
    ...
    for row in rows:
        if "Dereg Value" in row.text:
            ...
            if row_info:
                ...
                dereg = string_extraction(dereg)
                return dereg
            
    return np.nan  # Return np.nan if "Dereg Value" is not found
...
def get_omv_soup(soup):
    ...
    return np.nan  # Return np.nan if "OMV" is not found
...
def get_coe_value_soup(soup):
    ...
    return np.nan  # Return np.nan if "COE" is not found
    ...
    for row in rows:
        if "ARF" in row.text:
            ...
            if row_info:
                ...
                return arf
    return np.nan  # Return np.nan if "ARF" is not found
...
def get_power(url):
    power = float(url.find_all(class_='row_info')[10].text.strip().split(' kW')[0])
    return power
    ...
    for row in rows:
        if "Power" in row.text:
            ...
            if row_info:
                ...
                return power
    return np.nan  # Return np.nan if "Power" is not found
    ...
    for row in rows:
        if "Curb Weight" in row.text:
            ...
            if row_info:
                ...
                return weight
    return np.nan  # Return np.nan if "Curb Weight" is not found
        ...
        if "Owners" in row.text:
            ...
            if row_info:
                ...
                if "less than" in owner_text.lower():
                    ...
                    owner_num += 1
                return owner_num
    return np.nan  # Return np.nan if "Owners" is not found
